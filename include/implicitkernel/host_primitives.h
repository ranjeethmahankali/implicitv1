...
public:
   ...
   */
  virtual void render_data_size(size_t &nBytes, size_t &nEntities,
                                size_t &nSteps) const;
   ...
   * \param offsets The byte offsets of the simple entities (in the above
   * buffer). \param types The types of simple entities. \param steps The csg
   * steps to be performed on the simple entities.
   */
  void copy_render_data(uint8_t *&bytes, uint32_t *&offsets, uint8_t *&types,
                        op_step *&steps) const;
   ...
   * \param offsets The byte offsets of the simple entities (in the above
   * buffer). \param types The types of simple entities. \param steps The csg
   * steps to be performed on the simple entities. \param entityIndex For
   * internal use. \param currentOffset For internal use. \param reg For
   * internal use.
   */
  ...
  virtual void render_data_size_internal(
      size_t &nBytes, size_t &nSteps,
      std::unordered_set<entity *> &simpleEntities) const = 0;
 ...
 * with an operation. The operation can be either a boolean operation or a
 * blending operation.
 */
...
private:
  /**
   * \brief Construct a new comp entity object by combining two entities with an
   * operation. \param l First entity. \param r Second entity. \param op The
   * operation to be performed on the two entities.
   */
  comp_entity(ent_ref l, ent_ref r, op_defn op);

  /**
   * \brief Construct a new comp entity object by applying an operation to a
   * single entity. \param op The operation to be applied.
   */
...
public:
  ...
  virtual void
  render_data_size_internal(size_t &nBytes, size_t &nSteps,
   ...
   * \return ent_ref The reference to the new entity.
   */
  template <typename T> static ent_ref make_offset(T ent, float distance) {
    ent_ref ep(ent);
    op_defn op;
  ...
  /**
   * \brief Creates a new entity on the heap by combining the given entities
   * with a linear blend operation. \tparam T1 The type of the entities. \tparam
   * T2 The type of the entities. \param l The first entity. \param r The second
   * entity. \param p1 The first point for the interpolation. \param p2 The
   * second point for the interpolation. \return ent_ref The reference to the
   * new entity.
   */
  template <typename T1, typename T2>
  ...
  /**
   * \brief Creates a new entity on the heap by applying a smooth (s-shaped)
   * blend operation to the given entities. \tparam T1 The type of the first
   * entity. \tparam T2 The type of the second entity. \param l The first
   * entity. \param r The second entity. \param p1 The first point for
   * interpolation. \param p2 The second point for interpolation. \return
   * ent_ref The reference to the newly created entity.
   */
  template <typename T1, typename T2>
...
struct box3 : public simp_entity {
   ...
   */
  box3(float xcenter, float ycenter, float zcenter, float xhalf, float yhalf,
       float zhalf);
...
struct cylinder3 : public simp_entity {
   ...
   */
  cylinder3(float p1x, float p1y, float p1z, float p2x, float p2y, float p2z,
            float radius);
...
template <size_t N> struct polyface : public simp_entity {
  std::array<glm::vec3, N> vertices;

  polyface(const std::array<glm::vec3, N> &verts) : vertices(verts){};

  virtual uint8_t type() const { return ENT_TYPE_POLYFACE; };

  virtual size_t num_render_bytes() const {
    return sizeof(uint32_t) +       // Vertex count
           (sizeof(glm::vec3) * N); // Vertices;
  };

  virtual void write_render_bytes(uint8_t *&bytes) const {
    uint32_t nVerts = (uint32_t)N;
    std::memcpy(bytes, &nVerts, sizeof(uint32_t));
    bytes += sizeof(uint32_t);
    std::memcpy(bytes, vertices.data(), sizeof(glm::vec3) * N);
    bytes += sizeof(glm::vec3) * N;
  };
};
} // namespace entities
#pragma warning(pop)